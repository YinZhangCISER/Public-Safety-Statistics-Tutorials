...
Grandview supplies **82 percent** of the group's arrests, so pooling lets it carry 82 percent of the weight and averaging gives it a third. Pooling is right: the benchmark should answer "what rate did a person arrested by one of these agencies face", not "what is the average of three agency level numbers".

### The benchmark series, and the trap in it

![Three panels. The top panel maps the twelve agencies by similarity, coloured by whether they adopted the training, with lines joining each to its nearest peer; Riverbend's two nearest are both orange. The lower left panel shows Riverbend's twelve month trailing rate against two different peer benchmarks. The lower right panel shows the estimated programme effect under three choices of comparison group, against a dashed line at the true effect](Figures/fig_m12_comparing_agencies.png)

Riverbend adopted the de escalation training in July 2023. Its three nearest peers are **Cedar Falls, Summit County and Grandview**, and the first two **adopted the same programme**.
...
| Comparison group | Estimated effect on Riverbend |
|---|---|
...
With the trained peers left in, **the programme disappears entirely**. The benchmark fell for the same reason Riverbend fell, so the comparison subtracts the effect from itself.
...
<details>
<summary><b>2.</b> Riverbend's peer group is full of trained agencies. Is that bad luck with this dataset?</summary>
